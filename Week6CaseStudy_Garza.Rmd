...
```{r}
setwd("~/Dropbox/SMUDataScienceClass/Week6CaseStudy") # all important step of setting the working directory
...
MergeGDPData <- arrange(MergeGDPData,GDPinDollars)
MergeGDPData
```
...
```
The qplot shows an almost exponential curve with very few countries with high GDPs.
```{r}
